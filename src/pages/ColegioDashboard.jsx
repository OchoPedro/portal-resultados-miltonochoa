  ...
  return (
    <div style={{display:'flex', minHeight:'100vh', background:C.bg}}>
      ...
      <main style={{flex:1, padding: mobile ? '72px 16px 24px' : tablet ? '24px 20px' : '36px 40px', overflowY:'auto', minWidth: 0}}>
        ...
        {/* ══ NOTAS ESTUDIANTES POR COMPETENCIAS ══════════════ */}
        {tab==='comp_notas' && (() => {
          // Mapa rápido estudiante_id → datos del estudiante
          const estMap = {}
          ...
          // Filas filtradas por grado, salón y materia
          const filas = notasComp.filter(r => {
            const est = estMap[r.estudiante_id]
            if (!est) return false
            if (selectedGrado !== 'Todos' && est.grado !== selectedGrado) return false
          ...
          return students.length === 0 ? <EmptyState/> : (
            <Card>
              {/* Cabecera con filtro de asignatura */}
              <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start',
                flexWrap:'wrap', gap:12, marginBottom:20}}>
                <CardTitle sub={`${filas.length} registros · ${pruebaNombre}`}>
                  Notas Estudiantes por Competencias
                </CardTitle>
                <div style={{display:'flex', alignItems:'center', gap:8}}>
                  <span style={{fontSize:11, color:C.gray, fontFamily:'Inter'}}>Materia:</span>
