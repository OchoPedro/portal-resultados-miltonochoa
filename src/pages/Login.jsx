  ...
  const doLogin = async (roleFinal, u, p) => {
    ...
    try {
      if (roleFinal === 'estudiante') {
        ...
        if (err || !data) { setError('Usuario no encontrado.'); setLoading(false); return }
        if (data.password_hash !== p) { setError('Contraseña incorrecta.'); setLoading(false); return }
        ...
        if (err || !data) { setError('Código de colegio no encontrado.'); setLoading(false); return }
        if (data.password_hash !== p) { setError('Contraseña incorrecta.'); setLoading(false); return }
          ...
          <form onSubmit={handleLogin}>
            <div style={{ marginBottom: 16 }}>
              ...
              <input type="text" value={usuario} onChange={e => setUsuario(e.target.value)}
                ...
                required style={{ width: '100%', padding: '12px 16px',
                  border: `1px solid ${C.grayLt}`, borderRadius: 6,
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <input type="password" value={password} onChange={e => setPassword(e.target.value)}
                placeholder="••••••••" required style={{ width: '100%', padding: '12px 16px',
                  border: `1px solid ${C.grayLt}`, borderRadius: 6,
                  fontFamily: 'Inter', fontSize: 14, color: C.text,
                  background: C.bg, outline: 'none' }} />
            </div>
